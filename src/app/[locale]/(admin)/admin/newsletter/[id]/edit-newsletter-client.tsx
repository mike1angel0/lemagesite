...
import { updateNewsletterAction } from "@/lib/actions/newsletter";
      ...
      <div style={{ padding: "16px 20px", background: C.bgCard }}>
        ...
        {isEvent && (
          <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 4 }}>
            {item.eventDate && <p style={{ fontFamily: "system-ui, sans-serif", fontSize: 12, color: C.textPrimary, margin: 0 }}>&#128197; {item.eventDate}</p>}
            {item.eventLocation && <p style={{ fontFamily: "system-ui, sans-serif", fontSize: 12, color: C.textSecondary, margin: 0 }}>&#128205; {item.eventLocation}</p>}
          </div>
        )}
...
interface NewsletterData {
  ...
  replyTo: string | null;
  linkedContent: unknown[] | null;
}

export function EditNewsletterClient({
  newsletter,
  subscriberCount,
}: {
  ...
  const [audience, setAudience] = useState(newsletter.audience || "all");
  const [minimumTier, setMinimumTier] = useState(newsletter.minimumTier || "FREE");
  const [schedule, setSchedule] = useState(newsletter.scheduledAt ? "scheduled" : "immediately");
  const [sendDate, setSendDate] = useState(newsletter.scheduledAt ? newsletter.scheduledAt.split("T")[0] : "");
  const [sendTime, setSendTime] = useState(newsletter.scheduledAt ? newsletter.scheduledAt.split("T")[1]?.slice(0, 5) || "09:00" : "09:00");
  const [fromName, setFromName] = useState(newsletter.fromName || "Mihai Gavrilescu");
  const [replyTo, setReplyTo] = useState(newsletter.replyTo || "");
  const [embeddedContent, setEmbeddedContent] = useState<LinkedContentItem[]>(
    (newsletter.linkedContent as LinkedContentItem[]) || []
  );
  ...
  const [updateState, updateAction, updatePending] = useActionState(updateNewsletterAction, {} as AuthState);
  ...
  function handleSave() {
    updateAction(buildFormData(false));
  }

  function handleScheduleOrSend() {
    if (!subject.trim() || !body.trim()) return;
    updateAction(buildFormData(schedule === "scheduled"));
  }
  ...
  async function handleSendTest() {
    ...
    try {
      const res = await fetch("/api/newsletter/test", {
        ...
        body: JSON.stringify({ subject, previewText, body, fromName }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Send failed");
      setTestResult("Test email sent to your address!");
    } catch (err) {
    ...
    <>
      ...
      <div className="flex justify-between items-center px-8 py-4 border-b border-border">
        <div className="flex items-center gap-3">
          <Link href="/admin/newsletter" className="text-text-muted hover:text-text-primary transition-colors">
            <ArrowLeft size={18} />
          </Link>
          <h1 className="font-sans text-[18px] font-semibold text-text-primary">Edit Newsletter</h1>
          <span className={`rounded px-2 py-0.5 text-[10px] font-mono tracking-wide uppercase ${
            newsletter.status === "SENT" ? "bg-[#0D2818] text-[#6BBF7B]" :
            newsletter.status === "SCHEDULED" ? "bg-accent/10 text-accent" :
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 border border-border rounded-md px-4 py-2 font-sans text-sm text-text-secondary hover:border-accent-dim hover:text-text-primary transition-colors disabled:opacity-50"
          >
            <Send size={14} />
          ...
          {!isSent && (
            <>
              <button
                onClick={handleSave}
                disabled={updatePending || !subject.trim()}
              ...
              <button
                ...
                disabled={updatePending || !subject.trim() || !body.trim()}
      ...
      {updateState.error && (
        <div className="mx-8 mt-4 font-sans text-sm text-red-400 bg-red-400/10 border border-red-400/20 rounded px-4 py-2">{updateState.error}</div>
      )}
      {testResult && (
        <div className={`mx-8 mt-4 font-sans text-sm rounded px-4 py-2 ${
          testResult.includes("sent") ? "text-[#6BBF7B] bg-[#6BBF7B]/10 border border-[#6BBF7B]/20" : "text-red-400 bg-red-400/10 border border-red-400/20"
        }`}>{testResult}</div>
      )}
        ...
        <div className="flex-1 p-8 space-y-6 overflow-y-auto">
          <div className="space-y-2">
            ...
            <input
              ...
              disabled={isSent}
              className="w-full border border-border bg-transparent rounded py-2.5 px-3 font-sans text-sm text-text-primary placeholder:text-text-muted/50 focus:outline-none focus:border-accent transition-colors disabled:opacity-60"
            />
          </div>
          <div className="space-y-2">
          ...
          {showPreview ? (
            <div className="border border-border rounded overflow-hidden">
              ...
              <div style={{ background: C.bg, padding: 32, minHeight: 400 }}>
                <div style={{ maxWidth: 600, margin: "0 auto" }}>
                  ...
                  {body.trim() ? (
                    <div className="font-serif text-[15px] leading-[1.7]" dangerouslySetInnerHTML={{ __html: previewHtml }} />
                  ) : (
                    <p className="font-sans text-sm text-text-muted text-center py-12">Start writing to see a preview...</p>
                  )}
                  {embeddedContent.length > 0 && (
                    <div style={{ borderTop: `1px solid ${C.border}`, marginTop: 32, paddingTop: 24 }}>
                      <p style={{ fontFamily: "monospace", fontSize: 9, letterSpacing: 3, color: C.textMuted, textTransform: "uppercase", marginBottom: 16 }}>Featured Content</p>
                      {embeddedContent.map((item) => <EmbedCardPreview key={item.id} item={item} />)}
                    </div>
                  )}
                  <div style={{ textAlign: "center", borderTop: `1px solid ${C.border}`, marginTop: 32, paddingTop: 24 }}>
                    <p style={{ fontFamily: "system-ui, sans-serif", fontSize: 11, color: C.textMuted, margin: 0 }}>{footerText}</p>
            ...
            <>
              ...
              <textarea
                ...
                disabled={isSent}
                className="w-full min-h-[400px] bg-bg-card border border-border rounded p-6 font-serif text-base leading-relaxed text-text-primary placeholder:text-text-muted/50 resize-y focus:outline-none focus:border-accent transition-colors disabled:opacity-60"
              />
              {embeddedContent.length > 0 && (
                <div className="space-y-2">
        ...
        <div className="w-[320px] bg-bg-surface border-l border-border p-5 space-y-5 overflow-y-auto">
          <div className="space-y-2">
            ...
            <div className="relative">
              <select value={audience} onChange={(e) => setAudience(e.target.value)} disabled={isSent} className="w-full appearance-none border border-border bg-bg-card rounded py-2 px-3 pr-8 font-sans text-sm text-text-primary focus:outline-none focus:border-accent transition-colors disabled:opacity-60">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <select value={minimumTier} onChange={(e) => handleTierChange(e.target.value)} disabled={isSent} className="w-full appearance-none border border-border bg-bg-card rounded py-2 px-3 pr-8 font-sans text-sm text-text-primary focus:outline-none focus:border-accent transition-colors disabled:opacity-60">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <select value={schedule} onChange={(e) => setSchedule(e.target.value)} disabled={isSent} className="w-full appearance-none border border-border bg-bg-card rounded py-2 px-3 pr-8 font-sans text-sm text-text-primary focus:outline-none focus:border-accent transition-colors disabled:opacity-60">
            ...
            {schedule === "scheduled" && (
              <div className="flex gap-2 mt-2">
                <input type="date" value={sendDate} onChange={(e) => setSendDate(e.target.value)} disabled={isSent} className="flex-1 border border-border bg-bg-card rounded py-2 px-3 font-sans text-sm text-text-primary focus:outline-none focus:border-accent transition-colors disabled:opacity-60" />
                <input type="time" value={sendTime} onChange={(e) => setSendTime(e.target.value)} disabled={isSent} className="w-24 border border-border bg-bg-card rounded py-2 px-3 font-sans text-sm text-text-primary focus:outline-none focus:border-accent transition-colors disabled:opacity-60" />
              </div>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            <input type="text" value={fromName} onChange={(e) => setFromName(e.target.value)} disabled={isSent} className="w-full border border-border bg-bg-card rounded py-2 px-3 font-sans text-sm text-text-primary placeholder:text-text-muted/50 focus:outline-none focus:border-accent transition-colors disabled:opacity-60" />
          </div>

          <div className="space-y-2">
            <p className="font-mono text-[10px] text-text-muted tracking-[2px] uppercase">Reply-To</p>
            <input type="email" value={replyTo} onChange={(e) => setReplyTo(e.target.value)} disabled={isSent} placeholder="reply@example.com" className="w-full border border-border bg-bg-card rounded py-2 px-3 font-sans text-sm text-text-primary placeholder:text-text-muted/50 focus:outline-none focus:border-accent transition-colors disabled:opacity-60" />
          </div>
              ...
              {(["en", "ro"] as const).map((l) => (
                <button
                  ...
                  className={`flex items-center gap-1.5 flex-1 justify-center border rounded py-2 px-3 font-sans text-sm transition-colors ${
                    locale === l ? "border-accent text-accent bg-accent/10" : "border-border text-text-secondary hover:border-accent-dim hover:text-text-primary"
                  }`}
                >
              ...
              {!showContentPicker ? (
                ...
                <div className="space-y-2">
                  ...
                  <div className="max-h-[240px] overflow-y-auto space-y-1">
                    ...
                    {!searching && searchResults.length === 0 && <p className="font-sans text-xs text-text-muted py-2 text-center">No results</p>}
                    {searchResults.map((item) => {
                      const Icon = contentTypeIcons[item.type] || FileText;
                      return (
                        <button key={item.id} onClick={() => addContent(item)} className="w-full flex items-start gap-2 rounded py-2 px-2 hover:bg-bg-elevated transition-colors text-left">
                  ...
                  <button onClick={() => { setShowContentPicker(false); setSearchQuery(""); setSearchResults([]); }} className="font-sans text-xs text-text-muted hover:text-text-secondary transition-colors">Close</button>
                </div>
              )}
            </div>
          )}
