...
export default function AdminEditorNewPage() {
  ...
  const defaultPrompts: Record<string, string> = {
    Poetry: `Cute, playful ink and watercolor illustration for a poem titled "{TITLE}". {SUMMARY}. Whimsical, warm drawing with soft washes of color and a gentle, optimistic mood. An abstract, metaphoric human silhouette — faceless, dissolving into the scene, merging with nature or light. Playful details: floating petals, tiny stars, gentle light rays, soft bokeh dots. Warm palette: honey gold (#C9A962), soft amber (#C8944A), warm ivory (#F5EED8) as dominant tones, with touches of steel blue (#A8B4C8) and deep navy (#0B0E13) for contrast. Bright and uplifting, not dark. CRITICAL: The illustration must fill the ENTIRE frame edge-to-edge with no white space, no margins, no empty borders. No text, no watermarks, no color swatches.`,
    Essay: `Modern editorial illustration for "{TITLE}". {SUMMARY}. Bold, graphic style — clean shapes, flat color blocks, sharp compositions. A stylized human figure or scene, confident and contemporary. Think magazine cover art or New Yorker illustration. Warm palette: honey gold (#C9A962), soft amber (#C8944A), warm ivory (#F5EED8) as dominant tones, steel blue (#A8B4C8) and deep navy (#0B0E13) for structure and contrast. Bright, smart, visually striking. CRITICAL: The illustration must fill the ENTIRE frame edge-to-edge with no white space, no margins, no empty borders. No text, no watermarks, no color swatches.`,
    Research: `Clean, modern conceptual illustration for "{TITLE}". {SUMMARY}. Graphic style with precise geometric shapes and confident linework. A stylized human figure or abstract diagram-like composition. Warm palette: honey gold (#C9A962), soft amber (#C8944A), warm ivory (#F5EED8) as dominant tones, steel blue (#A8B4C8) and navy (#0B0E13) for structure. Intellectual, polished, editorial. CRITICAL: The illustration must fill the ENTIRE frame edge-to-edge with no white space, no margins, no empty borders. No text, no watermarks, no color swatches.`,
  };
  ...
  async function handleTranslate() {
    ...
    const ct = category === "Essay" ? "essay" : category === "Photography" ? "photography" : "poetry";
    try {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="w-[320px] bg-bg-surface border-l border-border p-5 flex flex-col gap-5 overflow-y-auto">
          ...
          {(category === "Poetry" || category === "Essay" || category === "Photography") && (
            <>
              <div>
          ...
          {/* Featured Image Upload (hidden for Photography — main area handles it) */}
          {category !== "Photography" && <div>
